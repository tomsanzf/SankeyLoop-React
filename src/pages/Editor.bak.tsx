...
import { Menu, Save, Upload, Download, RotateCcw, Video, Table as TableIcon, FileText, ChevronRight, Sun, Moon, GripVertical, HelpCircle, Copy, Clipboard } from 'lucide-react';
...
export default function Editor() {
  ...
  const copyTableToClipboard = () => {
    const tsv = editScenarioData.flows
      .filter(f => f.Source.trim() || f.Target.trim() || f.Value.trim() || f.Color.trim())
      .map(f => `${f.Source}\t${f.Target}\t${f.Value}\t${f.Color}`)
      .join('\n');
  ...
  const pasteTableFromClipboard = async () => {
    try {
      ...
      if (newFlows.length > 0) {
        if (newFlows[newFlows.length - 1].Source.trim() !== '' || newFlows[newFlows.length - 1].Target.trim() !== '') {
          newFlows.push({ Source: '', Target: '', Value: '', Color: '' });
        }
        updateScenario(editScenario, { flows: newFlows });
      }
    } catch (err) {
      alert('Unable to read clipboard automatically. Try clicking a cell in the table and pressing Ctrl+V.');
    }
  };

  const copyBetweenScenarios = (fromKey: string, toKey: string) => {
    const sourceScenario = scenarios[fromKey];
    if (!sourceScenario) return;
    
    const deepCopiedFlows = sourceScenario.flows.map(f => ({ ...f }));
    
    updateScenario(toKey, {
      flows: deepCopiedFlows,
      nodeColorOverrides: { ...(sourceScenario.nodeColorOverrides || {}) },
      nodePositions: { ...(sourceScenario.nodePositions || {}) },
      hasDraggedNodes: sourceScenario.hasDraggedNodes || false
    });
  };
              ...
              <div className={cn("flex-1 overflow-y-auto", inputMode !== 'guided' && "p-4")}>
                {inputMode === 'guided' ? (
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between text-[11px] text-[var(--text3)] py-0.5">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span>Table Controls:</span>
                        <button 
                          type="button"
                          onClick={copyTableToClipboard}
                          className="px-2 py-0.5 border border-[var(--border)] rounded bg-[var(--surface2)] text-[var(--text2)] hover:text-[var(--text)] hover:border-[var(--accent)] transition-all flex items-center gap-1 cursor-pointer"
                          title="Copy whole table to clipboard in Excel-compatible tab-separated format"
                        >
                          <Copy size={11} /> {copiedFeedback ? 'Copied!' : 'Copy Table'}
                        </button>
                        <button 
                          type="button"
                          onClick={pasteTableFromClipboard}
                          className="px-2 py-0.5 border border-[var(--border)] rounded bg-[var(--surface2)] text-[var(--text2)] hover:text-[var(--text)] hover:border-[var(--accent)] transition-all flex items-center gap-1 cursor-pointer"
                          title="Paste Excel-compatible data from clipboard"
                        >
                          <Clipboard size={11} /> Paste Table
                        </button>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        {editScenario === 'before' ? (
                          <button 
                            type="button"
                            onClick={() => {
                              if (confirm('Copy Before flows and layout to After? This will overwrite the After scenario.')) {
                                copyBetweenScenarios('before', 'after');
                              }
                            }}
                            className="px-2 py-0.5 border border-[var(--accent)]/50 rounded bg-[var(--accent)]/10 text-[var(--accent)] hover:bg-[var(--accent)] hover:text-white transition-all cursor-pointer text-[10px] font-semibold"
                            title="Copy flows and node layout from Before to After scenario"
                          >
                            Copy to After
                          </button>
                        ) : (
                          <button 
                            type="button"
                            onClick={() => {
                              if (confirm(`Copy Before flows and layout to current scenario (${editScenario})? This will overwrite this scenario.`)) {
                                copyBetweenScenarios('before', editScenario);
                              }
                            }}
                            className="px-2 py-0.5 border border-[var(--accent)]/50 rounded bg-[var(--accent)]/10 text-[var(--accent)] hover:bg-[var(--accent)] hover:text-white transition-all cursor-pointer text-[10px] font-semibold"
                            title={`Copy flows and node layout from Before to current scenario (${editScenario})`}
                          >
                            Copy from Before
                          </button>
                        )}
