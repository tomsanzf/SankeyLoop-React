...
export default function Editor() {
  ...
  const exportTransitionVideo = async () => {
    ...
    const pathData = paths.map((p, idx) => {
      let bbox = { x: 0, y: 0, width: 0, height: 0 };
      try { bbox = p.getBBox(); } catch(e) {}
      return { index: idx, bbox };
    }).filter(d => d.bbox.width >= 4);
    ...
    try {
      ...
      for (let f = 0; f < nFrames; f++) {
        ...
        const timeOffset = (f / fps);

        const clonedSvg = targetSvg.cloneNode(true) as SVGElement;
        clonedSvg.querySelectorAll('#sl-anim-style, .sl-anim-overlay, #sl-anim-defs').forEach(e => e.remove());
        
        const svgNS = 'http://www.w3.org/2000/svg';
        const defs = document.createElementNS(svgNS, 'defs');
        defs.id = 'sl-anim-defs';
        
        const clonedPaths = Array.from(clonedSvg.querySelectorAll('.sankey .links path')) as SVGPathElement[];
        
        pathData.forEach((d) => {
          const bbox = d.bbox;
          const linkBegin = (d.index / paths.length) * duration;
          let localTime = (timeOffset - linkBegin);
          while (localTime < 0) localTime += duration;
          localTime = localTime % duration;
          
          const progress = localTime / duration; 
          const currentX1 = (bbox.x - bbox.width) + (bbox.width * 2) * progress;
          const currentX2 = bbox.x + (bbox.width * 2) * progress;

          let opacity = 0;
          if (progress >= 0.4 && progress <= 0.6) {
            opacity = 1 - Math.abs(progress - 0.5) * 10;
          }

          if (opacity > 0) {
            const gid = `sl-g-exp-${d.index}`;
            const grad = document.createElementNS(svgNS, 'linearGradient');
            grad.id = gid;
            grad.setAttribute('gradientUnits', 'userSpaceOnUse');
            grad.setAttribute('x1', String(currentX1));
            grad.setAttribute('x2', String(currentX2));
            ...
            const cPath = clonedPaths[d.index];
            if (cPath) {
              const overlay = cPath.cloneNode(false) as SVGPathElement;
              overlay.setAttribute('fill', `url(#${gid})`);
              overlay.removeAttribute('stroke');
              overlay.style.opacity = String(opacity);
              cPath.parentNode?.insertBefore(overlay, cPath.nextSibling);
            }
          }
        });
